/**
 * The Daily Boss through the store (docs/design/BOSSES.md §1): keys charged before the fight,
...
/** A finished fight that did `damage` to the Easy tier's Gravemaw. */
...
describe('spending a key', () => {
  it('charges the key before the fight and refuses a third', () => {
    ...
    const first = actions.startBossFight('boss.gravemaw', 'easy');
    if (!first.ok) throw new Error(first.error.message);
    expect(first.value.encounterId).toBe('encounter.boss.gravemaw.easy');
    expect(first.value.keysLeft).toBe(1);
    expect(save(store).bosses['boss.gravemaw']?.keysUsed).toBe(1);

    expect(actions.startBossFight('boss.gravemaw', 'normal').ok).toBe(true);
    const third = actions.startBossFight('boss.gravemaw', 'easy');
    expect(third.ok).toBe(false);
  });

  it('refuses while the gate is closed, and refuses a tier that does not exist', () => {
    const { actions } = chronicle({ level: 9 });
    expect(actions.startBossFight('boss.gravemaw', 'easy').ok).toBe(false);
    expect(actions.startBossFight('boss.gravemaw', 'nightmare').ok).toBe(false);
...
describe('banking the damage', () => {
  ...
  it('remembers the team that set the record, and counts a kill', () => {
    ...
    const view = bossView(save(store), 'boss.gravemaw', T0);
...
describe('taking a chest', () => {
  it('pays what it holds, once', () => {
    ...
    const claimed = actions.claimBossChest('boss.gravemaw', 'easy', 5);
    ...
    expect(actions.claimBossChest('boss.gravemaw', 'easy', 5).ok).toBe(false);
    expect(actions.claimBossChest('boss.gravemaw', 'easy', 15).ok).toBe(false);
  ...
  it('mints the piece the kill chest promises', () => {
    ...
    const claimed = actions.claimBossChest('boss.gravemaw', 'easy', 100);
    if (!claimed.ok) throw new Error(claimed.error.message);
...
describe('the period turning over', () => {
  it('pays what was earned and left, then starts the day again', () => {
    ...
    actions.claimBossChest('boss.gravemaw', 'easy', 5);
    const goldBefore = held(store, 'gold');
    ...
    expect(next.bosses['boss.gravemaw']?.keysUsed).toBe(0);
    expect(next.bosses['boss.gravemaw']?.damage).toEqual({});
    // The record survives the reset.
    expect(next.bosses['boss.gravemaw']?.records['easy']?.damage).toBe(40_000);
...
describe('the view the screen reads', () => {
  ...
  it('is the boss the hub card names', () => {
    expect(content.bossById('boss.gravemaw')?.keyCurrency).toBe('key_daily');
    expect(content.bossById('boss.gravemaw')?.feature).toBe('daily_boss');
  });
});
