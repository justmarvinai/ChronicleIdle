  ...
  registry.currencies.forEach((raw, index) => {
    ...
    if (!parsed.success) {
      for (const issue of parsed.error.issues)
        issues.push({ path: `currencies[${index}].${issue.path.join('.')}`, message: issue.message });
      return;
    }
    const def = parsed.data;
    if (seen.has(def.id)) issues.push({ path: `currencies.${def.id}`, message: 'duplicate id' });
    seen.add(def.id);
    if (!refs.assetKeys.has(def.icon))
      issues.push({ path: `currencies.${def.id}.icon`, message: `unknown asset key ${def.icon}` });
    for (const key of [def.name, def.description]) {
      if (!refs.i18nKeys.has(key))
        issues.push({ path: `currencies.${def.id}`, message: `missing i18n key ${key}` });
    }
  });
  for (const id of CURRENCY_IDS)
    if (!seen.has(id))
      issues.push({ path: `currencies.${id}`, message: 'currency id declared but not defined' });
  return issues;
}
