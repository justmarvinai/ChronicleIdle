...
| a tutorial lesson | a `step()` in `src/content/tutorial/chapter_<n>.ts` (when it opens, what it points at, what finishes it) plus its line in `src/i18n/en/tutorial.ts`; a new spotlight means one name in `TUTORIAL_TARGETS` and one selector in `@ui/tutorial/targets.ts` — never a change inside a screen | `docs/tech/CONTENT_AUTHORING.md` §8, ADR-042 |
