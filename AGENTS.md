...
| a keep in the Dungeons | a `DungeonDef` in `src/content/dungeons/index.ts` (slug, order, the gear sets it holds, its keeper and warband, backdrop/glyph/surface) plus a `defineEnemy` keeper in `dungeons/keepers.ts` and its name, blurb and story in `src/i18n/en/dungeons.ts`; no stage is authored — the forty rungs, their scale, stars, rarity, energy and payouts all come from `src/content/balance/dungeon.ts`. The validator refuses a keep whose sets another keep already holds, and a set no keep holds at all | `docs/tech/CONTENT_AUTHORING.md` §16, `docs/design/DUNGEONS.md` |
| an item the Market sells | a `ConsumableDef` in `src/content/consumables/index.ts` (icon, rarity, and an `effect` from the six-kind union), a `single(order, slug, price)` on the shelf in `src/content/market/index.ts`, and its name and description in `src/i18n/en/market.ts`. A **new effect kind** is not content: add the arm to `ConsumableEffect`, the arm to `state/bag.ts`'s switch, the sentence to `market-view.ts`'s `outcomeLine`, and a test | `docs/tech/CONTENT_AUTHORING.md` §17, `docs/design/MARKET.md` §3 |
| a bundle on the Gem Market | a `GemShelfEntry` with `once: true` in `src/content/market/index.ts`, its two i18n keys, and **a comment giving the singly price** — the validator refuses a bundle priced at or above its parts, and the comment is what makes the price reviewable | `docs/tech/CONTENT_AUTHORING.md` §17.2, `docs/design/MARKET.md` §2.2 |
| a row on the Gold Market | nothing is authored: a row in `GOLD_MARKET_POOL` (`src/content/balance/market.ts`) with its weight, unit price and stock range. The stall is drawn from the pool by the hour, so there is no shelf to write | `docs/tech/CONTENT_AUTHORING.md` §17.2, `docs/design/MARKET.md` §1.1 |
| a day on the Rewards Calendar | a `{ day, tier, rewards }` row in `src/content/login/index.ts`. The board is exactly thirty days, the tiers are shuffled on purpose, and 28–30 must all be `legendary` while nothing before them may be — the validator refuses any other board. Raising a gem row moves `sim:economy`'s welcome band, which is the point of it | `docs/tech/CONTENT_AUTHORING.md` §17.3, `docs/design/LOGIN.md` §2 |
...
| a gear set | `src/content/sets/<slug>.ts` with the `set()` helper (2-piece / 4-piece bonus written as a passive), add it to `sets/index.ts`, its two i18n keys, every home settlement's `setPool` — **and exactly one keep's `sets`** in `src/content/dungeons/index.ts`, or the validator refuses it as unfarmable | `docs/tech/CONTENT_AUTHORING.md` §5, `docs/design/GEAR.md` §5, `docs/design/DUNGEONS.md` §2 |
