...
| a period boss (daily / weekly) | `src/content/bosses/<slug>.ts` with `defineBoss` — one kit, the tier table exactly as the design prints it, the chest ladder, `enrageEvery` — plus `bosses/index.ts` and its strings in `src/i18n/en/bosses.ts`; the tier enemies and their encounters are derived | `docs/tech/CONTENT_AUTHORING.md` §3.2, `docs/design/BOSSES.md` |
